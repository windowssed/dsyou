...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    <xsl:variable name="siteTitle">
      <xsl:choose>
        <xsl:when test="string-length(normalize-space(/rss/channel/title)) &gt; 0">
          <xsl:value-of select="normalize-space(/rss/channel/title)"/>
        </xsl:when>
    ...
    <xsl:variable name="siteDescription">
      <xsl:choose>
        <xsl:when test="string-length(normalize-space(/rss/channel/description)) &gt; 0">
          <xsl:value-of select="normalize-space(/rss/channel/description)"/>
        </xsl:when>
    ...
    <xsl:variable name="siteLink">
      <xsl:choose>
        <xsl:when test="string-length(normalize-space(/rss/channel/link)) &gt; 0">
          <xsl:value-of select="normalize-space(/rss/channel/link)"/>
        </xsl:when>
        <xsl:otherwise>/</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="pageLang">
      <xsl:choose>
        <xsl:when test="string-length(normalize-space(/rss/channel/language)) &gt; 0">
          <xsl:value-of select="normalize-space(/rss/channel/language)"/>
        </xsl:when>
    ...
    <xsl:variable name="iconHref">
      <xsl:choose>
        <xsl:when test="string-length(normalize-space(/rss/channel/image/url)) &gt; 0">
          <xsl:value-of select="normalize-space(/rss/channel/image/url)"/>
        </xsl:when>
        <xsl:otherwise></xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="appleTouchIconHref">
      <xsl:choose>
        <xsl:when test="string-length(normalize-space(/rss/channel/appleTouchIcon)) &gt; 0">
          <xsl:value-of select="normalize-space(/rss/channel/appleTouchIcon)"/>
        </xsl:when>
        <xsl:when test="string-length(normalize-space(/rss/channel/image/url)) &gt; 0">
          <xsl:value-of select="normalize-space(/rss/channel/image/url)"/>
        </xsl:when>
    ...
    <html lang="{$pageLang}">
      <head>
        <title>RSS Feed | <xsl:value-of select="$siteTitle"/></title>
      ...
      <body>
        ...
        <main>
          <ul class="posts">
            <xsl:for-each select="/rss/channel/item">
              ...
              <li class="post">
                <a class="post-link" href="{$itemLink}">
                  <span class="title">
                    <xsl:choose>
                ...
                <div class="meta">
                  <xsl:variable name="rawDate">
                    <xsl:choose>
                      <xsl:when test="string-length(normalize-space(updated)) &gt; 0">
        ...
        <div class="belonging">
          This is an RSS feed generated by
